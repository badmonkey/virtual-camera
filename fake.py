    ...
    def get_camera_values(self, status):
        print(
            "Real camera {} values are set as: {}x{} with {} FPS and video codec {}".format(
                status, self.get_frame_width(), self.get_frame_height(), self.get_frame_rate(), self.get_codec()
            )
        )
...
class FakeCam:
    def __init__(self, args) -> None:
        ...
        self.classifier = mp.solutions.selfie_segmentation.SelfieSegmentation(model_selection=args.select_model)
        ...
        self.fake_cam = pyfakewebcam.FakeWebcam(self.v4l2loopback_path, self.width, self.height)
    ...
    def compose_frame(self, frame):
        ...
        if self.no_background is False:
            ...
            background_frame = cv2.GaussianBlur(
                frame, (self.background_blur, self.background_blur), self.sigma, borderType=cv2.BORDER_DEFAULT
            )
...
def parse_args():
    ...
    parser.add_argument("-W", "--width", default=1280, type=int, help="Set real webcam width")
    parser.add_argument("-H", "--height", default=720, type=int, help="Set real webcam height")
    parser.add_argument("-F", "--fps", default=30, type=int, help="Set real webcam FPS")
    parser.add_argument("-C", "--codec", default="MJPG", type=str, help="Set real webcam codec")
    parser.add_argument("-w", "--webcam-path", default="/dev/video0", help="Set real webcam path")
    parser.add_argument("-v", "--v4l2loopback-path", default="/dev/video2", help="V4l2loopback device path")
    parser.add_argument(
        "--no-background", action="store_true", help="Disable background image and blur the real background"
    )
    parser.add_argument(
        "-b",
        "--background-image",
    ...
    parser.add_argument("--tile-background", action="store_true", help="Tile the background image")
    parser.add_argument(
        "--background-blur", default="21", type=int, metavar="k", help="The gaussian bluring kernel size in pixels"
    )
    parser.add_argument(
        "--background-blur-sigma-frac",
        default="3",
        type=int,
        metavar="frac",
    ...
    parser.add_argument(
        "--background-keep-aspect", action="store_true", help="Crop background if needed to maintain aspect ratio"
    )
    parser.add_argument("--no-foreground", action="store_true", help="Disable foreground image")
    ...
    parser.add_argument(
        "--background-mask-update-speed",
        default="50",
        type=int,
        help="The running average percentage for background mask updates",
    )
    parser.add_argument("--use-sigmoid", action="store_true", help="Force the mask to follow a sigmoid distribution")
    parser.add_argument(
        "--threshold",
        default="75",
        type=int,
    ...
    parser.add_argument(
        "--no-postprocess", action="store_false", help="Disable postprocessing (masking dilation and blurring)"
    )
    parser.add_argument(
        "--select-model",
        default="1",
...
https://gitlab.com/cvejarano-oss/cmapy/blob/master/docs/colorize_all_examples.md",
    ...
    parser.add_argument("--cmap-bg", default=None, type=str, help="Apply colour map to background using cmapy")
...
def getPercentageFloat(number):
    return min(max(number, 0), 100) / 100.0
...
def _log_camera_property_not_set(prop, value):
    print(
        "Cannot set camera property {} to {}. " "Defaulting to auto-detected property set by opencv".format(prop, value)
    )
